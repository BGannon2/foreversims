...
const slotKeys = { "Head": ["head"], "Neck": ["neck"], "Shoulders": ["shoulders"], "Back": ["back"], "Chest": ["chest"], "Wrist": ["wrist"], "Hands": ["hands"], "Waist": ["waist"], "Legs": ["legs"], "Feet": ["feet"], "Finger 1": ["finger1", "finger2"], "Finger 2": ["finger1", "finger2"], "Trinket 1": ["trinket1", "trinket2"], "Trinket 2": ["trinket1", "trinket2"], "Main Hand": ["main_hand", "off_hand"], "Off Hand": ["off_hand", "main_hand"], "Ranged / Relic": ["ranged", "relic"] };
...
function allowedItem(it, slot) {
  ...
  if ((it.requiredLevel || 0) > 60 || (it.requiredLevel || 0) < 45 || !["Rare", "Epic", "Legendary"].includes(it.quality)) return false;
...
function renderResultView(view) {
  ...
  if (view === "buffs" || view === "debuffs") { const rows = view === "buffs" ? x.buff_uptimes : x.debuff_uptimes; detail.innerHTML = `<h3>${view === "buffs" ? "Buff uptime" : "Debuff uptime"}</h3>` + Object.entries(rows).map(([n, v]) => `<div class="result-row"><span>${n.replaceAll("_", " ")}</span><div class="share-track"><div class="share-fill" data-bar-width="${v * 100}"></div></div><strong>${fmt(v * 100, 0)}%</strong></div>`).join(""); applyBarWidths(detail); return; }
