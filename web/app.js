...
async function init(){
  buildFields();
  try{
    const [response,itemResponse,wsResponse]=await Promise.all([fetch("/data/bootstrap.json?v=785b765"),fetch("/data/items.json?v=785b765"),fetch("/data/wowsims-import.json?v=785b765").catch(()=>null)]); if(!response.ok||!itemResponse.ok) throw new Error("Simulator data unavailable."); ForeverSim.warm(); state.wsData=wsResponse&&wsResponse.ok?await wsResponse.json():null; state.boot=await response.json(); const catalog=await itemResponse.json(); state.items=catalog.items; state.itemsById=new Map(state.items.map(item=>[item.id,item]));
