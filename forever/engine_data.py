...
ABILITIES = {
    ...
    "Demoralizing Shout": {"kind": "direct", "school": "physical", "cost": 10, "no_damage": True, "flat_threat": 43.2,
                            "provisional": "Rank 5 (max, level 54, spell 11556): 10 Rage, no cooldown beyond the GCD, 45 sec duration -- all confirmed via wago.tools DB2 (build 1.60.1.69913). The 43.2 flat threat per target hit isn't in DB2 (PvE threat coefficients aren't spell data) and comes from cross-checking wowsims/classic's own source (sim/warrior/demoralizing_shout.go): FlatThreatBonus = 0.4 * 2 * DemoralizingShoutLevel[5], where DemoralizingShoutLevel[5]=54 matches this sim's own sourced BaseLevel exactly. wowsims applies this once per target in the encounter with no cap (a plain loop over every target unit, no SpellTargetRestrictions row exists on this ability either) -- unlike Consecration's ground-effect AoE, which has a real packet-era target cap. Flagged missing from this sim entirely by a Mobalytics.gg Protection Warrior guide, which names it alongside Thunder Clap as a core pack-threat tool."},
